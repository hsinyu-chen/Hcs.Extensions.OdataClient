...
[{a=5},{a=4}]
### Select
for reduce complexity of expression parser `Select` can only apply once to the query, `select expression` is only for generate odata $select/$expand,then lib compile the expression for local projection use(`Enumerable.Select`),lib will insert nested member access null check for you (eq```csharp x=>new{x.Category.Name} => x=>new { Name=x.Category==null?x.Category.Name:default(string)}```), so don't worry about null check.
